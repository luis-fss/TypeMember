...
namespace UnitTests
{
    public class PropertyPathTests : TestBase
    {
        ...
        {
            var propertyName = Reflector.Property.GetPath<Blog>(b => b.Posts.Select(p => p.Author));
        ...
        {
            var propertyName = Reflector.Property.GetPath<Blog>(b => b.Posts.Select(p => p.Comments.Select(c => c.Member.UserName)));
